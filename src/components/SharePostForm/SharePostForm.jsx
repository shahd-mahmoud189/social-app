...
export default function SharePostForm({ toggle, isFormOpen, postId, setIsLiked, post }) {
  ...
  >
    ...
    <div className="flex items-center justify-between p-4 sm:p-5 border-b border-gray-100 shrink-0">
      <h2 className="text-lg sm:text-xl font-semibold text-gray-900">Share post</h2>
      <button 
        type="button" 
    ...
    <div className="p-4 sm:p-5 overflow-y-auto custom-scrollbar">
      ...
      {/* Post Preview Box */}
      <div className="bg-[#f9fafc] border border-gray-200 rounded-lg p-3 sm:p-5 flex flex-col gap-4">
        ...
        <div className="flex items-center gap-3">
          <img 
            src={post?.user.photo} 
            alt="user avatar" 
            className="w-8 h-8 sm:w-10 sm:h-10 rounded-full border border-gray-100"
          />
          <div className="min-w-0">
            <p className="font-semibold text-sm text-gray-950 truncate">{post?.user.name}</p>
        ...
        <div className="flex flex-col gap-2.5">
          ...
          {post?.image && (
            <img 
              src={post.image} 
              alt="" 
              className="mt-2 max-h-48 sm:max-h-64 w-full rounded-lg object-cover" 
            />
          )}
        </div>
    ...
    {/* Modal Footer */}
    <div className="flex justify-end gap-3 p-4 sm:p-5 border-t border-gray-100 shrink-0 bg-white">
      <button 
        type="button"
        onClick={() => toggle()} 
        className="px-4 sm:px-6 py-2 sm:py-2.5 bg-[#f0f2f5] hover:bg-gray-200 text-gray-800 text-sm font-semibold rounded-lg transition-colors"
      >
        Cancel
      </button>
      <button 
